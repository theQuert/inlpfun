...
$(TASK)-bin: $(TASK)
	for SPLIT in train $(DEV_NAME); do \
	  for EXT in source target; do \
	    /usr/bin/python3.7 -m fairseq.examples.roberta.multiprocessing_bpe_encoder --encoder-json $(HOME)/fairseq/encoder.json --vocab-bpe $(HOME)/fairseq/vocab.bpe --inputs "$(TASK)/$$SPLIT.$$EXT" --outputs "$(TASK)/$$SPLIT.bpe.$$EXT" --workers 60 --keep-empty; \
	  done; \
	done
